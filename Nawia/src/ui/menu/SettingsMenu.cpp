...
namespace Nawia::UI {
	...
	void SettingsMenu::drawScaleSlider(const float x, const float y, const float width, const Font& font) const
	{
	    ...
	    const Vector2 value_size = MeasureTextEx(font, value_text, font_size, spacing);
	    DrawTextEx(
	        font,
	        value_text,
		...
		const Vector2 max_size = MeasureTextEx(font, max_text, small_font, spacing);
	    DrawTextEx(font, max_text, {x + width - max_size.x, y + slider_height + Core::GlobalScaling::scaled(5.0f)}, small_font, spacing, GRAY);
	}

	void SettingsMenu::drawButton(const Rectangle& rect, const char* text, const bool is_hovered, const Font& font) const 
	{
