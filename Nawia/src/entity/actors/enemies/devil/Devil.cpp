...
namespace Nawia::Entity {
	...
	void Devil::handleDashingState(const float dt)
	{
			...
			if (const auto target = getTarget())
			{
				...
				if (dist_to_player <= DASH_HIT_RANGE)
				{
					...
					if (target->getType() == EntityType::Player)
						dynamic_cast<Player*>(target.get())->knockDown(static_cast<int>(DASH_DAMAGE * getDamageMultiplier()));
					else
						target->takeDamage(static_cast<int>(DASH_DAMAGE * getDamageMultiplier()));
		...
		const float move_amount = DASH_SPEED * getSpeedMultiplier() * dt;

		const float next_x = getX() + dir.x * move_amount;
		const float next_y = getY() + dir.y * move_amount;
	...
	{
		return {getX(), getAltitude() + _visual_jump_height, getY()};
	}
	...
	void Devil::spawnScorchedGroundHazard() {
		...
		config.damage_per_tick = static_cast<int>(6.0f * getDamageMultiplier());
	...
	void Devil::spawnShockwaveHazard() {
		...
		config.damage_per_tick = static_cast<int>(SHOCKWAVE_DAMAGE * getDamageMultiplier());
