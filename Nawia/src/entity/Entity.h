...
namespace Nawia::Entity {
	...
	class Entity {
	public:
		Entity(float start_x, float start_y, const std::shared_ptr<SDL_Texture>& texture, int max_hp);
		...
		bool isMouseOver(float mouse_x, float mouse_y, float cam_x, float cam_y) const;

		float getX() { return _pos->getX(); }
		float getY() { return _pos->getY(); }

		Core::Point2D getScreenPos(float mouse_x, float mouse_y, float cam_x, float cam_y) const;
